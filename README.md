...
- `duration` (string, optional): Extract specified duration in MM:SS format (e.g., "2:00")
...
Options:
  ...
  --duration <MM:SS>      Duration to extract (format: MM:SS, e.g., 2:00)
...
const result = await extractUniqueFrames({
  videoUrl: 'video.mp4',
  startTime: '1:00',  // Start at 1 minute
  duration: '0:30',    // Extract 30 seconds
  fps: 30
});
```
